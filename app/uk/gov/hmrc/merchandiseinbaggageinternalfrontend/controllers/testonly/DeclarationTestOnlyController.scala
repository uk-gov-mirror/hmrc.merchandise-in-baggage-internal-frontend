...
class DeclarationTestOnlyController @Inject()(
  mcc: MessagesControllerComponents,
  httpClient: HttpClient,
  views: DeclarationTestOnlyPage,
  declarationFoundView: DeclarationFoundTestOnlyPage,
  strideAuth: StrideAuthAction)(implicit val ec: ExecutionContext)
  ...
  def findDeclaration(declarationId: DeclarationId): Action[AnyContent] = strideAuth.async { implicit request =>
    declarationById(httpClient, declarationId)
      .map(declaration => Ok(declarationFoundView(declaration)))
      .recover({
  ...
  def onSubmit(): Action[AnyContent] = strideAuth.async { implicit request =>
    ...
    val newDeclaration: EitherT[Future, BusinessError, DeclarationIdResponse] =
      for {
        declarationRequest <- EitherT.fromOption(
                               Json
                                 .parse(bindForm.data(declarationFormIdentifier))
                                 .asOpt[DeclarationRequest],
                               InvalidDeclarationRequest)
    ...
    newDeclaration
      .fold(
        ...
        declarationIdResponse => Redirect(routes.DeclarationTestOnlyController.findDeclaration(declarationIdResponse.id))
      )
