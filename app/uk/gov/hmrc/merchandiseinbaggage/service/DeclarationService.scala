...
import uk.gov.hmrc.merchandiseinbaggage.model.api.{Amendment, AmountInPence, DeclarationId, DeclarationType, Goods, JourneyDetails, Paid}
...
object DeclarationService {
  ...
  def totalPayment(decAmount: AmountInPence, amendments: Seq[Amendment]): AmountInPence = {
    def paidAmendment: Seq[Amendment] = amendments.filter(_.paymentStatus.contains(Paid))
